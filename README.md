...
Define AOI using PRODES polygons (Rondônia region).

Download Sentinel-1 GRD and Sentinel-2 L2A data within target temporal range.

Apply SAR calibration, terrain correction, and filtering.

Perform optical preprocessing and co-registration to SAR geometry.

Handle persistent cloud cover using pix2pix GAN-based imputation.

Construct and validate multi-temporal 4D data cubes ready for model input.

Upload outputs to shared cloud storage for model training (Vasist) and validation (Suhruth).
